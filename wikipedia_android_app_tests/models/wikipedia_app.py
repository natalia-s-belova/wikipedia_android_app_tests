...
class Wikipedia:
    ...
    def search_for_article(self, text):
        with step(f'Enter query <{text}> to the search field'):
    ...
    def verify_first_article_title_in_the_list(self, text):
        with step(f'Verify article is first in the list and title is: <{text}>'):
            results = browser.all((AppiumBy.ID, 'org.wikipedia.alpha:id/page_list_item_title'))
            results.should(have.size_greater_than(0))
            results.first.should(have.exact_text(text))
    ...
    def clear_search_field_and_enter_text(self, text):
        ...
        with step(f'Enter new query <{text}> to the search field'):
            browser.element((AppiumBy.ID, 'org.wikipedia.alpha:id/search_src_text')).type(text)
    ...
    def search_history(self, *text):
        with step('Tap on search field'):
            browser.element((AppiumBy.ACCESSIBILITY_ID, 'Search Wikipedia')).click()
        with step('Verify previously entered queries are shown in Search History (recent on the top)'):
    ...
    def verify_phrase_exists_in_article(self, text):
        ...
        with step(f'Enter text <{text}> and verify only 1 match found'):
            browser.element((AppiumBy.ID, 'org.wikipedia.alpha:id/search_src_text')).type(text)
            browser.element((AppiumBy.ID, 'org.wikipedia.alpha:id/find_in_page_match')).should(have.exact_text('1/1'))

    def verify_article_is_not_found(self):
        with step('Verify "No results" message is shown'):
            browser.element((AppiumBy.ID, 'org.wikipedia.alpha:id/results_text')).should(have.exact_text('No results'))
    ...
    def verify_article_added_to_saved(self, text):
        ...
        with step(f'Close hint and verify article title is <{text}>'):
            browser.element((AppiumBy.ID, 'org.wikipedia.alpha:id/buttonView')).click()
            browser.element((AppiumBy.ID, 'org.wikipedia.alpha:id/page_list_item_title')).should(have.exact_text(text))
    ...
    def verify_hiding_featured_article(self):
        with step('Verify Featured article is shown at the top of Home Page'):
            browser.element((AppiumBy.ID, 'org.wikipedia.alpha:id/view_card_header_title')).should(
                have.exact_text('Featured article'))
